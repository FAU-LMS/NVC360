...
class IntraNoAR(CompressionModel):
    ...
    def forward(self, x, pos, q_scale=None):
    ...
    def encode_decode(self, x, pos, q_scale, output_path=None, pic_width=None, pic_height=None):
        # pic_width and pic_height may be different from x's size. X here is after padding
        # x_hat has the same size with x
        if output_path is None:
            return self.forward(x, pos, q_scale)

        assert pic_height is not None
        assert pic_width is not None
        q_scale, q_index = get_rounded_q(q_scale)
        compressed = self.compress(x, pos, q_scale)
        ...
        decompressed = self.decompress(bit_stream, pos, height, width, q_index / 100)
    ...
    def compress(self, x, pos, q_scale):
        curr_q = self.get_curr_q(q_scale)
    ...
    def decompress(self, bit_stream, pos, height, width, q_scale):
        curr_q = self.get_curr_q(q_scale)
