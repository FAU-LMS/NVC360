...
from DCVCHEM.src.models.video_model import DMC
...
class DCVCHEMController(CompressionModelController):

    def __init__(self, keyframe_model_path, video_model_path, quality, gop, device='cpu'):
        super().__init__()
        ...
        p_frame_y_q_scales, p_frame_mv_y_q_scales = DMC.get_q_scales_from_ckpt(video_model_path)
        ...
        with torch.no_grad():
            for i in tqdm(range(frames), desc="Encoding", unit="frame"):
                ...
                if i % self._gop == 0:
                    ...
                    encoded = self._video_net.compress(x_cur, dpb, self._mv_y_q_scale, self._y_q_scale)
                    self.write_uints(bitstream, (len(encoded['bit_stream']),))
                    self.write_bytes(bitstream, encoded['bit_stream'])
                    dpb = self._video_net.decompress(dpb, encoded['bit_stream'], height, width,
        ...
        with torch.no_grad():
            for i in tqdm(range(frames), desc="Decoding", unit="frame"):
                if i % self._gop == 0:
                    ...
                    num_bytes = self.read_uints(bitstream, 1)[0]
                    bit_stream = self.read_bytes(bitstream, num_bytes)
                    dpb = self._video_net.decompress(dpb, bit_stream, height, width,
                                                     self._mv_y_q_index / 100, self._y_q_index / 100)['dpb']
